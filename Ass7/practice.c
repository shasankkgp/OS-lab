...
void create_workers ( pthread_t *tid, tinfo *param )
{
   ...
   pthread_attr_init(&attr);    // setting the properties of threads to default state 
   pthread_attr_setdetachstate(&attr, PTHREAD_CREATE_JOINABLE);    // specifing customised properties of threads
...
void do_work ( pthread_t *tid, tinfo *param )
{
   ...
   /*
    ...
    Blocks Until Thread Terminates: The calling thread (in this case, the master thread) will wait at this point in execution until the specified thread completes. It's essentially a synchronization point.
    Collects Return Value: If the second parameter (retval) is not NULL, it stores the return value of the terminated thread. In your example, it's passing NULL because the code doesn't need the return values.
    Releases Resources: It frees the resources (memory, kernel structures) associated with the terminated thread. Without calling pthread_join, these resources would remain allocated, causing a resource leak.
    Detects Thread Status: The return value of pthread_join indicates whether the join operation was successful.
   */
   for (i=0; i<N; ++i) {
...
int main ()
{
   ...
   create_mutex();    // create mutexes and condition variables
   init_arrays();    // just fill the random values in S array
   print_arrays();    // printing function 
   create_workers(tid,param);    // setting default properties of threads and creating threads
   do_work(tid,param);    // simulating some work for 10 seconds
   print_arrays();      // printing the arrays
   wind_up();       // destroying mutexes and condition variables
   exit(0);
}
