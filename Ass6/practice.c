...
main()
{
	...
	*/
	...
	semctl(semid1, 0, SETVAL, 0);         // checks for empty byffer 
	semctl(semid2, 0, SETVAL, 1);         // checks for full buffer 
	...
	*/
	if (fork() == 0) {
		...
		while (count) {    // actually we have to add one more semaphore for count, to maintain integrity

			P(semid1);     // waits if the buffer is empty 
			fp = fopen("datafile","r");
			fscanf(fp, "%d", &data);
			printf("\t\t\t\t Consumer reads %d\n",data);
			fclose(fp);
			V(semid2);    // signals the semaphore handling full buffer 
			count--;
		}
		...
		while (count) {
			...
			P(semid2);     // wait if the buffer is full 
			fp = fopen("datafile","w");
			fprintf(fp, "%d\n", data);
			printf("Producer writes %d\n", data);
			data++;
			fclose(fp);
			V(semid1);    // signals to semaphore maintaining empty buffer 
			count--;
		}
		wait(&status);
